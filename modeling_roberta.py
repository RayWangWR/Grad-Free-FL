...
class RobertaEmbeddings(nn.Module):
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, prompt_embedding=None):
        ...
        if prompt_embedding is not None:
            ...
            pre_padding = torch.zeros(bsz, 1, prompt_dim).to(prompt_embedding.device)
...
class RobertaModel(RobertaPreTrainedModel):
    ...
    def forward(
        ...
        embedding_output = self.embeddings(
            input_ids=input_ids, position_ids=position_ids, token_type_ids=token_type_ids, inputs_embeds=inputs_embeds, prompt_embedding=prompt_embedding
        )
...
class RobertaForMaskedLM(RobertaPreTrainedModel):
    ...
    def __init__(self, config, n_prompt_tokens, inference_framework='pt', onnx_model_path=None):
        ...
        self.descrete = None
#         if self.descrete is not None:
        offset = 1000
        indices_offset = torch.tensor(list(range(offset, offset + self.n_prompt_tokens)))
        prompt_offset = self.roberta.embeddings.word_embeddings.weight[indices_offset].detach()
        self.prompt_offset = nn.Parameter(prompt_offset, requires_grad=False)
    ...
    def compute_natural_prompt(self):
        weight = self.roberta.embeddings.word_embeddings.weight
        weight_m = weight - torch.mean(weight, dim=0, keepdim=True)
#         breakpoint()
#         logits = torch.matmul(self.prompt_embedding, torch.transpose(weight, 0, 1))
        logits = torch.matmul(self.prompt_embedding, torch.transpose(weight if not self.m else weight_m, 0, 1))
        probs = torch.nn.functional.softmax(logits*2, dim=-1)
        if self.descrete:
            indices = torch.argmax(probs, dim=-1)
            prompt_embedding = weight[indices]
        else:
            prompt_embedding = torch.matmul(probs, weight)
        return prompt_embedding - self.prompt_offset

    def set_prompt_embedding(self, prompt_embedding):
        self.prompt_embedding = prompt_embedding.to(self.device)
...
#             prompt_embedding = self.prompt_embedding
        ...
        if self.descrete is None:
            prompt_embedding = self.prompt_embedding
        else:
            prompt_embedding = self.compute_natural_prompt()

        if True: #prompt_embedding is not None:
...
class RobertaForIntrinsicTuning_fed(RobertaPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids", r"predictions.decoder.bias"]
    _keys_to_ignore_on_load_unexpected = [r"pooler"]

    def __init__(self, config, n_prompt_tokens, intrinsic_dim, n_clients, inference_framework='pt', onnx_model_path=None):
        super().__init__(config)
        ...
        self.intrins_embedding_fed = torch.nn.ParameterList( [nn.Parameter(torch.Tensor(intrinsic_dim))
                                                        for _ in range(self.n_clients)] )
        
        nn.init.normal_(self.intrinsic_embedding, mean=0, std=0.5)
        nn.init.normal_(self.intrinsic_embedding_fed, mean=0, std=0.5)
        self.linear = torch.nn.Linear(intrinsic_dim, n_prompt_tokens * config.hidden_size, bias=False)
...
#         print('total param is {}'.format(total_param))
        prompt_param = 0
#         for name, param in self.intrins_vector.named_parameters():
#             prompt_param += param.numel()
        prompt_param += self.intrins_embedding.numel()
        print(f'params: {prompt_param}/{bert_param}={prompt_param/bert_param}')
    ...
    def init_A(self, sigma1=1):
        embedding = self.roberta.get_input_embeddings().weight.clone().cpu()
        ...
        mu = 0.0
        std = std_hat / (np.sqrt(self.intrins_dim) * sigma1)
    ...
    def forward(
        ...
        if self.sep is False:
            intrinsic_embedding = self.intrinsic_embedding
        elif self.sep is True:
            intrinsic_embedding = self.intrinsic_embedding_fed[self.client_id]
        prompt_embedding = self.linear(intrinsic_embedding)
